...
const GanttChart: React.FC<GanttChartProps> = ({ tasks, isForPdf = false }) => {
  ...
  const totalDurationDays = getDaysDiff(projectStartDate, projectEndDate) + 1;
  ...
  const getWeekMarkers = () => {
      ...
      currentDate.setDate(currentDate.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1)); // Start on Monday

      while (currentDate.getTime() <= projectEndDate.getTime() + (7 * 24 * 3600 * 1000)) {
          const offsetDays = getDaysDiff(projectStartDate, currentDate);
          const offsetPercent = (offsetDays / totalDurationDays) * 100;
  ...
  const getDayMarkers = () => {
      const markers = [];
      const increment = totalDurationDays > 10 ? 2 : 1;
      for (let i = 0; i < totalDurationDays; i += increment) {
          const currentDate = new Date(projectStartDate.getTime() + i * 24 * 3600 * 1000);
          const offsetPercent = (i / totalDurationDays) * 100;
          markers.push({
  ...
  const timeMarkers = (() => {
      if (totalDurationDays > 90) {
          return getMonthMarkers();
      }
      if (totalDurationDays > 15) {
          return getWeekMarkers();
      }
      return getDayMarkers();
  })();
                        ...
                        <div key={task.id} className="flex items-center py-3">
                            ...
                            <div className="flex-grow relative h-8 bg-slate-100 rounded-md">
                                ...
                                >
                                    ...
                                    {!isForPdf && (
                                        <div className="absolute -top-14 left-1/2 -translate-x-1/2 hidden group-hover:block p-3 text-xs text-white bg-slate-800 rounded-md shadow-lg z-10 w-52 text-left">
                                            <p className="font-bold border-b border-slate-600 pb-1 mb-1 truncate">{task.taskName}</p>
                                            <p><span className="font-semibold">Período:</span> {task.startDate} a {task.endDate}</p>
                                            <p><span className="font-semibold">Responsável:</span> {task.assignee}</p>
